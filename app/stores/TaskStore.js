...
class TaskStore {
  ...
  constructor() {
    ...
    this.tasks = Immutable.OrderedMap({});
    ...
    this.bindListeners({
      handleIncrement: TaskActions.INCREMENT,
      handleDecrement: TaskActions.DECREMENT,
      handleCreate: TaskActions.CREATE,
      handleDestroy: TaskActions.DESTROY,
      handleTyping: TaskActions.TYPING
    });
  }

  bootstrap() {
    if (!Immutable.OrderedMap.isOrderedMap(this.tasks)) {
      this.tasks = fromJSOrdered(this.tasks);
    }
    this.newTask = '';
  }

  handleIncrement(id) {
    const topic = this.tasks.get(id);
    const count = topic.get('count');
    this.tasks = this.tasks.set(id, topic.set('count', count + 1));
    this.emitChange();
  }
  ...
  handleCreate(data) {
    const id = data.id;
    this.tasks = this.tasks.set(id, Immutable.fromJS(data));
    this.emitChange();
  }
